...
import os
os.environ["DB_CONNECTION"] ="postgresql://zengqinglong:zqlthedever@192.168.1.96:5432/monolithic_crystal"

import tensorflow as tf
from hotpot.utils.tf_gpu import USEGPU
USEGPU(0)

raw=Database().read_sql('''SELECT
	*
FROM
	hits
...
xxx=(FuncList(combinations([30, 90, -30, -90], 1))
...
def close_enough(cart_1, cart_2):
    ...
    if coordinate_close.x==True and coordinate_close.y==True and coordinate_close.z==True:
        return True
    else:
        return False
...
for crystalID in range(80):
    raw=Database().read_sql(f'''SELECT
        *
    FROM
        hits
    ...
    result = [(close_enough(local_pos_2_global_pos(local_pos, ma), global_pos), ma) for ma in move_args]
    tmp.append([crystalID, *FuncList(result).filter(lambda i: i[0] is True).to_list()[0][1]])
