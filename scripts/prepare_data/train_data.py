...
sipm_counts_n_position = tf.cast(
    ...
    .concatenate_with(
        FuncArray(swa.sipm_center_pos.shrink((1, 2)).rollaxis(1, 4).array[:, :, :, :3]),
        axis=3,
    )
    .concatenate_with(
        FuncArray(tf.transpose(swa.gamma_2_counts, perm=[0, 2, 1, 3]) / 10), axis=3
    )
    .concatenate_with(
        FuncArray(swa.sipm_center_pos.shrink((1, 2)).rollaxis(1, 4).array[:, :, :, 3:]),
        axis=3,
...
source_position = tf.cast(
    FuncArray(swa.sample_df[["sourcePosX", "sourcePosY", "sourcePosZ"]]).to_tensor(),
    tf.float32,
)

save_as_npy(
    f"sipm_counts_n_position_experiment_{args.experiment_id}.npy",
