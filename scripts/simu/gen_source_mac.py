...
def cylinder_source(ind, centre=(0, 0), radius=1.0, activity=0.0):
...
def d_phan_section(radius, num_layers):
    ...
    def nth_layer_deltas(nth_layer):
        ...
        delta_ys = (
            np.full_like(delta_xs, (nth_layer - 1) * delta_y)
            + SECTION_DISTANCE_TO_CENTER
        )
        return np.vstack([delta_xs, delta_ys]).T

    delta_x, delta_y = radius_to_delta()
    nth_layer_coord = np.vstack(
        [nth_layer_deltas(i).tolist() for i in range(1, num_layers + 1)]
    )

    return np.vstack([nth_layer_coord.T, np.full_like(nth_layer_coord[:, 0], radius)]).T


def rotate_2d(point, theta):
    return np.matmul(
        point, [[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]]
    )


STANDARD_PHANTOM = np.array(
    [[0.4, 8], [1.25, 3], [1.0, 4], [0.75, 5], [0.625, 6], [0.5, 7]]
)
...
x_result = np.vstack(
    [
        np.concatenate(
            [
                rotate_2d(d_phan_section(radius, num_layer)[:, :2], angle),
                d_phan_section(radius, num_layer)[:, 2:],
                np.full_like(
                    d_phan_section(radius, num_layer)[:, 2:], dose(radius, cylinder_vol)
                ),
            ],
            axis=1,
        )
...
x_result = np.hstack(
    [
        np.reshape(np.array(range(1, x_result.shape[0] + 1)), [x_result.shape[0], 1]),
        x_result,
    ]
)
...
print(
    "\n".join(
        [
            cylinder_source(int(ind), [x, y], radius, activity)
            for ind, x, y, radius, activity in x_result
        ]
    )
)
