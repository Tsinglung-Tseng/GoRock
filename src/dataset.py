...
class PhantomDataset(Dataset):
    def __init__(self, config):
        ...
        self.x_image_size = tuple(i//self.config.downsampling_ratio for i in self.y_image_size)
        
        self.num_test = int(self.num_dataset_samples * self.config.test_portion)
        self.num_train = self.num_dataset_samples - self.num_test
        
        self.y_train, self.y_test = self.phantom[:self.num_train][..., tf.newaxis], self.phantom[self.num_train:][..., tf.newaxis]
