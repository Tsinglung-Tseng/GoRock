...
class CMD:
    ...
    def sub_finder(task_config):
        return CMD(
            f"""find {task_config["work_dir"]} -type d -name {task_config["sub_pattern"]}"""
        )
    ...
    def check_task_output(task_config):
        return CMD(
            f"""find {task_config["work_dir"]} -type d -name "{task_config["sub_pattern"]}" \! -exec test -e '"""
            + "{}"
    ...
    def make_work_dir(task_config):
        return (
            CMD(f"mkdir {task_config['work_dir']}")
            .atom(CMD(f"cd {task_config['work_dir']}"))
            .atom(CMD(f"mkdir task_{task_config['number_of_subs']}_subs"))
            .atom(CMD(f"cd task_{task_config['number_of_subs']}_subs"))
            .atom(
                CMD(
                    f"seq 0 {task_config['number_of_subs']-1} | xargs -i mkdir sub."
                    + "{}"
                )
            )
        )
...
def prepare_mac(task_config):
    geometry_path = (
        Database()
        .read_sql(
            f"""select path from experiment_config where experiment_id={task_config['geometry_id']};"""
        )
        .to_numpy()[0][0]
        + "/sub.0/"
    )
    source_path = (
        Database()
        .read_sql(
            f"""select path from experiment_config where experiment_id={task_config['source_id']};"""
        )
        .to_numpy()[0][0]
        + "/sub.0/"
    )
...
def load_source(task_config):
    return CMD(
        f"""find {task_config['work_dir']}/task_{task_config['number_of_subs']}_subs -type d -name "sub.*" | xargs -i cp /tmp/"""
        + "{sbatch_task.sh,Geometry.mac,Source.mac}"
        + " {}"
    ).atom(
        CMD(
            f"""find {task_config['work_dir']}/task_{task_config['number_of_subs']}_subs -type d -name "sub.*" | xargs -i cp /home/zengqinglong/optical_simu/optical_common/*"""
            + " {}"
        )
    )
...
def submit_to_slurm(task_config):
    return (
        CMD.check_task_output(task_config)
        .pipe(CMD.sort_numerically())
        .pipe(
            CMD(
                """xargs -L 1 bash -c 'cd "$0" && pwd && sbatch --cpus-per-task=8 --mem 20240 sbatch_task.sh'"""
            )
        )
    )
