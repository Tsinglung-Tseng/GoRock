...
class Quaternion:
    ...
    def from_axis_angle(cls, axis: UnitVector, angle):
        ...
        return Quaternion(a=tf.cos(angle / 2), b=ijk.x, c=ijk.y, d=ijk.z)

    def to_rotation_matrix(self):
        return tf.stack(
            [
                [
                    1 - 2 * tf.square(self.c) - 2 * tf.square(self.d),
                    2 * self.b * self.c - 2 * self.a * self.d,
                    2 * self.a * self.c + 2 * self.b * self.d,
                ],
                [
                    2 * self.b * self.c + 2 * self.a * self.d,
                    1 - 2 * tf.square(self.b) - 2 * tf.square(self.d),
                    2 * self.c * self.d - 2 * self.a * self.b,
                ],
                [
                    2 * self.b * self.d - 2 * self.a * self.c,
                    2 * self.a * self.b + 2 * self.c * self.d,
                    1 - 2 * tf.square(self.b) - 2 * tf.square(self.c),
                ],
            ]
        )
